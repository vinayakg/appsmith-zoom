export default {
  ...
  getCustomernames: () => {
    const namesArray = ZoomUsers.selectedRows.map((row) => row.name);
  ...
  sendSMSFunc: async () => {
    ...
    if (mode.includes("SMS")) {
      const selectedNums1 = ZoomUsers?.selectedRows?.map((row) => row?.sms_number);
  ...
  sendWhatsAppFunc: async () => {
    ...
    if (mode.includes("WhatsApp")) {
      const selectedNums1 = ZoomUsers?.selectedRows?.map(
  ...
  selectedMessages: () => {
    return getMessages.data.filter((word) =>
      word.type
        .toLowerCase()
        .includes(Meeting_Details.selectedOptionValue.toLowerCase())
    );
  },
